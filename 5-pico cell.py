...
# 设置文件名的基本路径和文件名格式
base_file_path = "Micro cell_"  # 文件名的基本路径
...
for file_number in range(1, i):  # 假设您有文件从 1 到 i 编号
    ...
    Demarcation_point = total_points - (m * 200)  # 减去Micro cell 对应的人数 1Gbps ————5Mbps 200 / day
    if Demarcation_point > 0:  # 判断是否需要Micro cell进行支持
        num_clusters = int(np.ceil(Demarcation_point / (m * 20)))  # 计算聚类的数量，取最小大于等于Demarcation_point / (m * 200)的正整数
...
cluster_centers_df.to_csv('all_picocell_centers.csv', index=False)
...
ax.set_title('Pico cell overlay')
...
for file_counter, center in enumerate(new_centers, start=1):  # 使用enumerate生成序号
    ...
    # 在图上显示编号
    plt.text(center[0], center[1], str(file_counter), ha='center', va='center', fontsize=10, color='green')
...
plt.scatter(x_coordinates, y_coordinates, s=0.5, marker='.', color='blue')
