...
def main():
    with open('all_picocell_centers.csv', 'r') as csvfile:
    ...
    ax.set_title('Schematic diagram of Pico cell')
    ...
    for hexagon_index, (center_x, center_y) in enumerate(hexagon_centers, start=1):
        ...
        hexagon_csv_filename = f'Pico cell_{hexagon_index}.csv'
        ...
        ax.text(center_x, center_y, str(hexagon_index), ha='center', va='center', fontsize=10, color='black')

        color = (np.random.random(), np.random.random(), np.random.random())  # Generate a random color
        plt.scatter(*zip(*contained_points), marker='.', color=color, s=10)
    ...
    with open('total_pico_counts.csv', 'w', newline='') as csvfile:
